...
struct impas
{
    int game_mode=0;
    int difficulty=1;
    int latura=4;                     ///latura tablei/nr patratele per margine
...
int main()
{
    PlaySound(TEXT("TOTO.wav"), NULL, SND_FILENAME | SND_ASYNC);
    draw_main();
    return 0;
}
void auto_move()
...
void game_loop()
{
    ...
    coord clk;
    const int Last_WINDOW = initwindow(500, 300, "Game Over");
    setcurrentwindow(Last_WINDOW);
    setfillstyle(SOLID_FILL,CYAN);
    setcolor(WHITE);
    setbkcolor(CYAN);
    bar(0,0,500,500);
    ...
    char text[50];
    if(singleton.game_mode==1)///player vs AI
    {
        if(singleton.rand!=singleton.player)
        {
            sprintf(text,"Jucatorul a castigat");
        }
        else
        {
            sprintf(text,"Ai-ul a castigat");
        }
    }
    else
    {
        sprintf(text,"Jucatorul %d a castigat", (singleton.rand^1)+1);
    }

    settextstyle(COMPLEX_FONT,HORIZ_DIR,4);
    settextjustify(CENTER_TEXT,CENTER_TEXT);

    outtextxy(250,50,text);

    settextstyle(COMPLEX_FONT,HORIZ_DIR,3);
    settextjustify(LEFT_TEXT,CENTER_TEXT);
    outtextxy(20,200,"joc nou");

    settextjustify(CENTER_TEXT,CENTER_TEXT);
    outtextxy(250,200,"main menu");

    settextjustify(RIGHT_TEXT,CENTER_TEXT);
    outtextxy(480,200,"exit");


    clk=get_clickXY();
    while(buton_apasat(clk,GAME_OVER)==0)
    {
        clk=get_clickXY();
    }
    if(buton_apasat(clk,GAME_OVER)==1)///joc nou
    {
        closegraph(Last_WINDOW);
        setcurrentwindow(WINDOW);

        singleton.initializare(singleton.latura);
        game_loop();
    }
    else if(buton_apasat(clk,GAME_OVER)==2)///inapoi la meniu
    {
        closegraph(Last_WINDOW);
        setcurrentwindow(WINDOW);
    ...
    else if(buton_apasat(clk,GAME_OVER)==3)///EXIT
    {
        return;
    }
...
int buton_apasat(coord clk,int fereastra)
{
    ...
    if(fereastra==GAME_OVER)
    {
        /**
            ...
            outtextxy(480,200,"exit");
        */
        if(abs(clk.y-200)<=textheight("jocnoumainmenuexit ")/2)
        {
            if(clk.x>=20&&clk.x<=20+textwidth("joc nou"))
                {
                    return 1;
                }
            else if(abs(clk.x-250)<=textwidth("main menu")/2)
            {
                return 2;
            }
            else if(clk.x>=480-textwidth("exit")&&clk.x<=480)
            {
                return 3;
            }
                return 0;
